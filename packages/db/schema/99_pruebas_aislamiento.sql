...
SET ROLE lis_app;
SET lis.empresa_id = '1'; SELECT count(*) AS ve_empresa_1 FROM core.paciente;
SET lis.empresa_id = '2'; SELECT count(*) AS ve_empresa_2 FROM core.paciente;
SET lis.empresa_id = '';  SELECT count(*) AS sin_contexto  FROM core.paciente;
RESET ROLE;
...
SET ROLE lis_app; SET lis.empresa_id = '1';
...
SET ROLE lis_app; SET lis.empresa_id = :'lab'; SET lis.usuario_id = :'usuario_id';
SELECT core.entregar(:lab, :enl, 'solicitud', '{"pruebas":["hemograma"]}'::jsonb,
                     :usuario_id) IS NOT NULL AS se_entrego;
SET lis.empresa_id = :'lab'; SELECT count(*) AS ve_quien_mando   FROM core.mensaje;
SET lis.empresa_id = :'cli'; SELECT count(*) AS ve_quien_recibio FROM core.mensaje;
RESET ROLE;

\echo ''
\echo '=== 6 · Y ninguna puede escribir la fila de la otra ==============='
\echo '    esperado: ERROR row-level security policy'
SET ROLE lis_app; SET lis.empresa_id = :'lab';
